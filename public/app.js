...
// ===== STATE =====
let state={sel:['DE','RO','FR','ES'],yr:2026,th:'dark',vw:'cal',sm:null,op:false,q:'',modal:null,acOpen:null,heatAll:false,
  // Working days calc
...
function renderBridgeDays(){
  ...
  return`<div class="tool-card" style="max-width:900px"><h2>🌉 Bridge Day Optimizer — ${bdYear}</h2><p class="desc">Smart leave planning: see exactly which days to take off to maximize consecutive days away. Only high-value opportunities shown.</p><div class="tool-input"><div><label>Country</label><select onchange="setState({bdCountry:this.value})">${countryOpts}</select></div><div><label>Year</label><select onchange="setState({bdYear:+this.value})">${yearOpts}</select></div></div><div style="margin-bottom:12px;font-size:11px;color:var(--dm)">${cx?.f} ${cx?.n} — ${bridges.length} opportunities</div>${items}</div>`;
}
...
function renderCompare(){
  ...
  sel.forEach((cc,ci)=>{
    ...
    const rows=[
      {label:'📅 Total holidays',key:'total'},
      {label:'💼 Weekday holidays',key:'weekday'},
      {label:'🏖️ Long weekends (Mon/Fri)',key:'longWE'},
      {label:'⚡ Mid-week (Wed)',key:'midWeek'},
      {label:'🌉 Free 4+ day blocks',key:'bridges'},
      {label:'✅ Clear work weeks',key:'avWeeks',green:true}
    ];
    rows.forEach(r=>{
...
c2=`<button style="background:var(--ab);color:var(--at);border:none;padding:6px 14px;border-radius:7px;font-size:11px;font-weight:600;cursor:pointer;margin-bottom:12px" onclick="setState({acOpen:null})">← Back</button><div style="display:flex;align-items:center;gap:10px;margin-bottom:12px"><span style="font-size:28px">${c.f}</span><div><div style="font-size:18px;font-weight:700">${c.n}</div><div style="font-size:12px;color:var(--dm)">${sorted.length} holidays in ${yr}</div></div><button class="bbtn bbtn-ics" style="margin-left:auto;padding:6px 12px;font-size:10px" onclick="generateICS(['${acOpen}'],${yr})">📥 Download .ics</button></div><div class="cp-stats"><div class="cp-stat"><div class="num">${sorted.length}</div><div class="lbl">Public holidays</div></div><div class="cp-stat"><div class="num">${wdCount.wd}</div><div class="lbl">Working days</div></div><div class="cp-stat"><div class="num" style="color:var(--gn)">${goodBridges.length}</div><div class="lbl">Bridge opportunities</div></div></div>${bridgeHtml?'<h3 style="font-size:12px;font-weight:700;margin:10px 0 4px">🌉 Best Bridge Days</h3>'+bridgeHtml:''}<h3 style="font-size:13px;font-weight:700;margin:12px 0 6px">📋 Holiday List</h3><div class="ac-list">${li}</div><h3 style="font-size:13px;font-weight:700;margin:14px 0 6px">📅 Calendar</h3><div class="ac-mini-cal">${mc}</div>`;}else{c2=CT.map(c=>{const cnt=H[c.c]?Object.keys(H[c.c]).length:0;return`<div class="ac-country" onclick="setState({acOpen:'${c.c}'})"><span class="flag">${c.f}</span><span class="cname">${c.n}</span><span class="cnt">${cnt} holidays</span></div>`;}).join('');}return`<div class="modal-overlay" onclick="if(event.target===this)setState({modal:null,acOpen:null})"><div class="modal"><button class="modal-close" onclick="setState({modal:null,acOpen:null})">✕</button><h2 style="font-size:18px;font-weight:700;margin-bottom:4px">🌍 Non-Working Days — All European Countries — ${yr}</h2><p style="font-size:11px;color:var(--dm);margin-bottom:14px">Tap any country for full profile, holiday list, calendar, and bridge days.</p>${c2}</div></div>`;}

// ===== MAIN RENDER =====
function render(){
const{sel,yr,th,vw,sm,op,q,modal}=state;
...
let html=`<header style="text-align:center"><h1>🗓️ European Holiday Calendar</h1><p class="sub">Compare holidays across ${CT.length} European countries · Plan sprints · Maximize time off</p></header>`;
// Year + theme controls
html+=`<div class="ctrls"><select onchange="setState({yr:+this.value,sm:null})" style="padding:4px 10px;border-radius:7px;border:1px solid var(--id);background:var(--ib);color:var(--ac);font-size:13px;font-weight:700;cursor:pointer;outline:none">${YEARS.map(y=>`<option value="${y}"${y===yr?' selected':''}>${y}</option>`).join('')}</select><span class="sep"></span>${[['dark','🌙'],['pink','🌸'],['light','☀️']].map(([i,l])=>`<button class="thm${th===i?' on':''}" onclick="setState({th:'${i}'})">${l}</button>`).join('')}</div>`;
// Country pills
html+=`<div class="pills">${sel.map((c,i)=>{const x=CT.find(z=>z.c===c);return`<button class="pill" style="background:${PAL[i%PAL.length]}18;color:${PAL[i%PAL.length]}" onclick="setState({sel:state.sel.filter(x=>x!=='${c}')})"><span style="font-size:13px">${x?.f}</span>${x?.n}<span class="x">✕</span></button>`;}).join('')}<button class="add-btn" onclick="setState({op:!state.op})">+ Add</button></div>`;
if(op){html+=`<div class="picker"><input type="text" placeholder="Search countries…" value="${q}" oninput="setState({q:this.value})"><div class="picker-grid">${ft.map(c=>{const on=sel.includes(c.c);return`<button class="picker-btn${on?' on':''}" onclick="setState({sel:state.sel.includes('${c.c}')?state.sel.filter(x=>x!=='${c.c}'):[...state.sel,'${c.c}']})"><span style="font-size:12px">${c.f}</span>${c.n}${on?'✓':''}</button>`;}).join('')}</div><button class="picker-done" onclick="setState({op:false,q:''})">Done</button></div>`;}

// Navigation — two rows
html+=`<div class="nav-wrap"><div class="tabs">${[['cal','📅 Calendar'],['ins','💡 Insights'],['av','✅ Available']].map(([v,l])=>`<button class="${vw===v?'on':''}" onclick="setState({vw:'${v}'})">${l}</button>`).join('')}</div><br><div class="tabs tools">${[['wd','🧮 Working Days'],['bd','🌉 Bridge Days'],['ta','🔔 Team Alerts'],['cmp','📊 Compare Years']].map(([v,l])=>`<button class="${vw===v?'on':''}" onclick="setState({vw:'${v}'})">${l}</button>`).join('')}</div></div>`;

// === VIEW CONTENT ===
if(vw==='cal'){html+=`<div class="mf"><button class="${sm===null?'on':''}" onclick="setState({sm:null})">All</button>${MN.map((m,i)=>`<button class="${sm===i?'on':''}" onclick="setState({sm:${i}})">${m}</button>`).join('')}</div><div class="cal-grid${sm!==null?' single':''}">${calH}</div>`;}
if(vw==='ins'){const bH=sel.map((c,i)=>{const x=CT.find(z=>z.c===c),n=tot[c]||0,mx=Math.max(...Object.values(tot),1);return`<div class="bar-row"><span style="font-size:14px">${x?.f}</span><div style="flex:1"><div style="display:flex;justify-content:space-between;margin-bottom:2px"><span style="font-size:11px;font-weight:600">${x?.n}</span><span style="font-size:11px;font-weight:700;color:${PAL[i%PAL.length]}">${n}</span></div><div class="bar-track"><div class="bar-fill" style="width:${(n/mx*100)}%;background:${PAL[i%PAL.length]}"></div></div></div></div>`;}).join('');const ovH=ov.map(o=>`<div class="ov-item"><div style="font-size:10px;font-weight:700;color:var(--ac);margin-bottom:2px">${o.d} ${FME[o.m]}</div>${o.hs.map(h=>`<div style="font-size:10px;color:var(--sb)">${h.f} <span style="color:${h.cl};font-weight:600">${h.cn}</span> — ${h.n}</div>`).join('')}</div>`).join('');const wH=wst.map((w,i)=>`<div class="wst-item" style="background:${i<3?'#ef444412':'var(--dy)'};border-color:${i<3?'#ef444422':'var(--bd)'}"><span style="font-size:11px;font-weight:600;color:${i<3?'#f87171':'var(--tx)'}">Wk ${w.s.d} ${MN[w.s.m]}</span><span style="padding:2px 6px;border-radius:5px;font-size:10px;font-weight:700;background:${i<3?'#ef444422':'#f59e0b18'};color:${i<3?'#f87171':'#f59e0b'}">${w.imp}</span></div>`).join('');const lH=lng.slice(0,10).map(l=>`<div class="lng-item"><div style="font-size:11px;font-weight:700;color:#10b981">${l.d} ${FME[l.m]} <span style="font-size:9px;font-weight:400;color:#6ee7b7">${l.tp}</span></div><div style="font-size:10px;color:var(--sb)">${l.hs.map(h=>h.f+' '+h.n).join(', ')}</div></div>`).join('');html+=`<div class="igrid"><div class="icard"><h3>📊 Holidays / Country — ${yr}</h3>${bH}</div><div class="icard"><h3>🔀 Overlaps (${ov.length})</h3><div style="max-height:250px;overflow-y:auto">${ovH||'<p style="color:var(--dm);font-size:10px">No overlaps.</p>'}</div></div><div class="icard"><h3>⚠️ Worst Weeks</h3>${wH}</div><div class="icard"><h3>🏖️ Long Weekends</h3><div style="max-height:250px;overflow-y:auto">${lH}</div></div></div>`;}
if(vw==='av'){html+=`<div class="icard" style="background:var(--cd);border-radius:12px;border:1px solid var(--bd);padding:16px"><h2 style="font-size:15px;font-weight:700;margin-bottom:3px">✅ Everyone Available — ${yr}</h2><p style="font-size:11px;color:var(--dm);margin-bottom:10px">Full work weeks with zero holidays across selected countries.</p>${av.length?`<p style="font-size:11px;color:#10b981;font-weight:700;margin-bottom:8px">${av.length} clear weeks</p>`:'<p style="color:var(--dm);font-size:11px">No clear weeks found.</p>'}<div class="av-grid">${av.map(w=>`<div class="av-card"><div>Wk ${w.s.d} ${FME[w.s.m]}</div></div>`).join('')}</div></div>`;}
...
// Bottom buttons
html+=`<div class="bottom-btns"><button class="bbtn bbtn-pdf" onclick="exportPDF(state.sel,state.yr)">📄 Export PDF</button><button class="bbtn bbtn-ics" onclick="generateICS(state.sel,state.yr)">📥 Download .ics</button><button class="bbtn bbtn-heat" onclick="setState({modal:'heat'})">🔥 Heat Distribution</button><button class="bbtn bbtn-all" onclick="setState({modal:'all',acOpen:null})">🌍 All European Countries</button></div>`;
